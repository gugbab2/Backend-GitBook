...
<table><thead><tr><th width="129">프로세스</th><th>컨테이너 내부 (Namespace PID)</th><th>널이 보는 호스트 PID (Real PID)</th></tr></thead><tbody><tr><td>Spring 컨테이너의 JVM</td><td>1</td><td>예 : 1000</td></tr><tr><td>DB 컨테이너의 mysqld</td><td>1</td><td>예 : 2000</td></tr><tr><td>캐시 컨테이너의 redis</td><td>1</td><td>예 : 3000</td></tr><tr><td>호스트</td><td>컨테이너가 아니기 때문에 할당 X</td><td>예 : 100</td></tr></tbody></table>
...
* **컨테이너 100개를 띄워도, 이 읽기 전용 데이터는 딱 1개만 메모리에 올려두고 사용한다.**
...
>
> * 디스크 (HDD/SSD)
>   * 각각의 컨테이너가 읽기 전용 계층의 수정을 시도할 때만 해당 파일이 쓰기 가능 계층으로 복사되어 디스크를 사용한다.&#x20;
...
>   * 예를 들어, Spring Boot 의 JVM Heap, Stack, 로그 버퍼, 변수 등 프로세스가 실행 중 생성/수정하는 데이터는 각 컨테이너마다 독립적으로 RAM 에 할당되어 관리된다.&#x20;
>
...
>
> **여기서 문제점은 리눅스 시스템은 생성 가능한 최대 PID 수를 설정해 놓는데, 일반적으로는 32,768 개 컨테이너의 경우에는 더 낮게 제한될 수 있다.**&#x20;
>
