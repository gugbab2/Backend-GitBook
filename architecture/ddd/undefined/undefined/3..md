...
* 초기에는 단일 서버에 단일 DBMS 로 서비스를 제공하는 것이 가능하다. 하지만 사용자가 몰리면, 사연스럽게 부하를 분산하기 위해서 하위 도메인별로 시스템을 분리하기 시작한다. 이 과정에서 하위 도메인마다 다른 DBMS 를 사용할 때도 있다.&#x20;
...
>
> * 데이터베이스나 외부 데이터 소스에 접근하기 위한 추상화된 인터페이스를 제공하는 객체를 의미한다.&#x20;
>   * 즉, 데이터 조회, 추가, 수정, 삭제 같은 작업을 수행하는 비지니스 로직과 데이터 엑세스 로직을 분리하기 위해서 사용된다.&#x20;
>
> #### DAO vs Repository&#x20;
