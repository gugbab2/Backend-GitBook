...
* 템플릿을 사용하 마치 인자 인것 처럼 사용하는 것을 바로 일반화 프로그래밍(Generic Programming) 이라고 부른다. \
...
* 참고로 위처럼 배열을 감싸는 **wrapper 클래스를 만들어 마치 배열 처럼 사용한다면(물론 그렇게 사용하기 위해 \[] 연산자도 오버로드 해야한다) 배열을 사용함으로 발생하는 문제들을 많이 해결할 수 있**
