...
* **바로 {} 는 암시적 형변환을 불허한다는 점이다.**&#x20;
* 아래 코드를 보자 \
  **-> () 경우는 암시적 형변환을 허용하지만, {} 경우는 컴파일 에러를 발생시킨다.**&#x20;
...
* 아래 코드를 살펴보면, 컴파일 에러가 발생한다. \
  ...
  **(initializer\_list 타입으로 형변환이 불가능한 경우만 이러한 문제가 발생하지 않는다)**\
  **-> **_**다시 한번 말하지만, 컴파일러는 {} 생성의 경우 initializer\_list 타입 생성자를 최우선시 하기 한다!**_
...
* 다음의 경우에는 `A b{3, 1.5};` 가 string 타입으로 형변환이 불가하기 때문에 `A(int x, double y);` 생성자를 호출하게 된다.&#x20;
...
* 물론 이 문제는 C++14 에 추가된 리터럴 연산자를 통해서 해결할 수 있다. \
  \-> 다음에 코드에서는 initializer\_list\<std::string> 으로 추론되ㅏㄴ다.&#x20;
