...
#### 아래와 같은 STL 의 문제점으로 회사내부에서 STL 대체품들을 만들기도 한다.&#x20;
...
int main()
{
    ...
    for(std::vector<int>::reverse_iterator iter=scores.begin(); i<scores.end(); ++iter)
    {
        // 반복자는 사실상 포인터이다. 
        std::cout << *iter << " ";
    }    
...
* **메모리 복사 : 삽입되는 인덱스 뒤에 데이터는 모두 복사되어야 한다.**&#x20;
...
* **이전에 c++ 은 개체배열 생성 시 각 요소(힙영역)마다 주소값이 아닌 개체 멤버변수를 저장한다고 했다.**&#x20;
...
* 백터에 개체를 직접 저장시 다음과 같이 2가지 문제가 발생한다. \
