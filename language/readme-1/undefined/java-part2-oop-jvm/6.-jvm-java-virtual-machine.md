...
> #### ⚙️ Runtime Constant Pool (중요!)
>
> Java에서 `.class` 파일은 클래스의 메서드, 필드, 상수, 버전 등의 정보를 저장한 **Constant Pool Table**을 포함하고 있다. 이 `.class` 파일이 JVM에 의해 로딩될 때, 해당 정보를 바탕으로 **클래스별 Runtime Constant Pool**이 생성된다.
>
> 🔍 Runtime Constant Pool의 역할
>
> * 각 클래스마다 하나씩 생성된다.
> * 상수(숫자, 문자열 리터럴), 심볼릭 참조(클래스 이름, 메서드 이름 등)를 포함한다.
> * **심볼릭 참조**는 런타임 시점에 JVM이 해석하여 **실제 메모리 참조(주소)**&#xB85C; 치환된다.\
>   이를 **Resolution 단계**라고 부른다.
>
> 📦 Runtime Constant Pool 위치: PermGen vs Heap
>
> * Java 7 이하
>   * 저장 위치 :  PermGen (Method Area)
>   * 관리 방식 :  고정 크기, GC에 영향 적음
>   * 관련 메모리 : 클래스 메타데이터와 함께 저장
> * Java 8 이상&#x20;
>   * 저장 위치 : JVM Heap 영역
>   * 관리 방식 : GC 대상, 크기 유동적
>   * 관련 메모리 : intern pool, 상수 정보 등만 힙에서 따로 관리
>
> 🔄 동적 특성
>
> * Runtime Constant Pool은 **JVM 실행 중에도 동적으로 확장될 수 있음**\
>   (예: `String.intern()` 을 통해 새로운 문자열이 풀에 추가될 수 있음)
>
> ```java
> String str = new String("hello");
> String interned = str.intern(); // intern pool에 등록 (없으면 추가)
> ```
