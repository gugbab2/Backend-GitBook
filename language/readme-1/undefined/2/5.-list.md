...
* 이렇게 하면 `MyList` 를 사용하는 클라이언트 코드인 `BatchProcessor` 를 전혀 변경하지 않고, 원하는 리스트 전략 을 런타임에 지정할 수 있다.
...
의존관계는 크게 컴파일 탕미 의존관계와 런타임 의존관계로 나눌 수 있다.&#x20;
...
정리하면 이론적으로는 `MyLinkedList` 가 편균 추가(중간 삽입) 에 있어서 더 효율적일 수 있지만, 현대 컴퓨터 시스템의 메모리 접근 패턴, CPU 캐시 최적화 등을 고려할 때 `MyArrayList` 가 실제 사용 환경에서 더 나은 성능을 보여주는 경우가 많다.
...
#### `List` 는 객체들의 순서가 있는 컬렉션을 나타내며, 같은 객체의 중복 저장을 허용한다. 이 리스트는 배열과 비슷하지만, 크기가 동적으로 변화하는 컬렉션 을 다룰 때 유연하게 사용할 수 있다.
...
현대 컴퓨터 시스템의 메모리 접근 패턴,CPU 캐시 최적화, 메모리 고속복사 등을 고려할 때 \
`ArrayList` 가 실제 사용 환경에서 더 나은 성능을 보여주는 경우가 많다.
