...
> 참고 : CPU 에 어떤 프로그램이 얼마만큼 실행될지는 운영체제가 결정하는데, 이것을 스케줄링(scheduling) 이라한다. 이때 단순히 시간으로만 작업을 분할하지는 않고, CPU 를 최대한 활용할 수 있는 다양한 우선순위와 최적화 기법을 사용한다. &#x20;
...
* **이 코드를 한줄씩 실행하는 주체가 스레드이다.** \
  **-> 이렇듯 프로세스의 코드를 실행하는흐름을 스레드라고 한다.**&#x20;
...
* **CPU 코어 갯수와 스레드의 갯수를 일치시키거나, 스레드 대기 상태를 고려해 CPU 코어 갯수보다 1개더 많게 스레드 갯수를 맞춘다면 이상적이라고 생각할 수 있다.**&#x20;
* **하지만, 이상적이지 못한 경우도 있다..**
...
* 사용자의 요청 하나를 처리하는데, 스레드는 CPU 1% 정도 사용하고, 대부분 데이터베이스 서버에 어떤 결과를 조회하면서 기다린다고 가정하자. 이때는 스레드는 CPU 를 거의 사용하지 않고 대기한다. \
