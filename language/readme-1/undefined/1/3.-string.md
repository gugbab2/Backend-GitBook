...
> #### 참고 - 문자열 리터럴이 문자열 풀(Intern Pool)에 언제 등록이 되는가?&#x20;
>
> 1. 문자열 리터럴이 컴파일 타임에 .class 파일 내 Constant Pool 에 저장되고
> 2. JVM 로딩 시 메서드 영역 or 메타스페이스에 위치한 Runtime Constant Pool 에 저장된다.
> 3. **실제로 사용시 문자열 풀(Intern Pool) 에 등록되어 캐시하여 사용할 수 있다.**
