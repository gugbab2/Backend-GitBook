...
* CPU 코어의 연산 속도는 메모리 접근 속도보다 훨씬 빠릅니다. 이로 인해 코어는 메모리 대기(stall), 파이프라인 버블 등으로 내부 실행 유닛이 유휴 상태가 되는 순간이 빈번합니다.
* 하드웨어 스레드는 이 유휴 실행 자원을 낭비하지 않기 위해, 하나의 물리 코어 안에 레지스터 세트 등 아키텍처 상태를 복제하여 여러 스레드의 명령어를 동시에 공급하는 기술입니다. 한 스레드가 ALU를 쓰는 동안 다른 스레드가 FPU나 로드/스토어 유닛을 사용하는 식으로, 코어 내부 자원의 활용률을 높입니다.
* 인텔의 하이퍼스레딩(Hyper-Threading)이 대표적이며, 싱글 코어에 하드웨어 스레드가 2개라면 OS는 이를 2개의 **논리 프로세서(logical processor)**&#xB85C; 인식합니다. 다만 실행 유닛 자체가 늘어나는 것은 아니므로 실제 듀얼 코어와 동등한 성능을 내지는 않으며, 일반적으로 약 15\~30% 수준의 처리량 향상을 기대할 수 있습니다.

**커널 레벨 스레드 (네이티브 스레드):**&#x20;

* OS 커널이 직접 생성하고 관리하는 스레드입니다. 컨텍스트 스위칭에 커널이 개입하므로 비용이 발생합니다.

**유저 레벨 스레드:**&#x20;

* 프로그래밍 언어 레벨에서 추상화한 스레드입니다. `threading.Thread`가 여기에 해당합니다.
