# 웹 서버는 어떻게 Python 코드를 실행하게 되었는가 — CGI, WSGI, ASGI의 발전과 실전 배포
...
* **비동기 논블로킹**: async/await 구문을 사용하여 한 번에 많은 수의 동시 요청을 효과적으로 처리할 수 있다. \
  I/O 대기 중에 다른 요청을 처리하므로, 단일 프로세스에서도 수천 개의 동시 연결이 가능하다.
...
Uvicorn은 매우 빠른 ASGI 서버이며, `--workers` 옵션을 통해 멀티 워커 실행도 지원한다. 그러나 Uvicorn의 멀티 워커 기능은 비교적 단순한 구현으로, 프로세스 관리 능력이 제한적이다. 또한 각 워커는 단일 이벤트 루프로 동작하므로, CPU 집약적 작업이 이벤트 루프를 블로킹하면 해당 워커의 전체 성능이 저하될 수 있다.

Gunicorn은 이러한 한계를 보완하는 프로세스 관리자 역할을 수행한다. 오랜 기간 프로덕션 환경에서 검증된 성숙한 프로세스 매니저로서, 두 서버를 함께 사용하면 다음과 같은 이점을 얻는다.
...
* **안정성 및 관리 효율:** 워커 프로세스 충돌 시 자동 재시작, 정상 종료(graceful shutdown), 워커 타임아웃 관리, 시그널 핸들링 등 프로덕션 환경에 필요한 견고한 프로세스 관리 기능을 제공한다.&#x20;
  * **이는 Uvicorn 자체 멀티 워커 대비 핵심적인 차별점이다.**
...
다만 최근 Uvicorn의 멀티 워커 기능도 지속적으로 개선되고 있어, 소규모\~중규모 서비스에서는 Uvicorn 단독으로도 충분한 경우가 많다. **Gunicorn + Uvicorn 조합은 대규모 프로덕션 환경에서 안정성을 우선시할 때 선택하는 패턴이다.**
