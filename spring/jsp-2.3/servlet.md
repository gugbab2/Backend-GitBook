...
* HttpServlet는 GenericServlet과 마찬가지로 상속받으면 간단하게 서블릿을 실행할 수 있다. 그리고 우리가 대부분 사용하는 서블릿은 HttpServlet 추상 클래스를 상속받은 클래스다.
* GenericServlet 이 init(), destroy()까지 구현했다면 HttpServlet는 service()까지 구현되있다.
* 다만 이름에서 알 수 있듯이 GenericServlet은 http뿐 아니라 다양한 프로토콜에 대응이 가능하다. 즉 Http 프로토콜을 이용하지 않을 때도 사용이 가능하다.
* 하지만 HttpServlet는 웹개발서 주로 쓰는 프로토콜이 http라서 이를 편하게하려고 만들었다. 그래서 HttpServlet는 Http 프로토콜에만 한정적으로 사용할 수 있다.
