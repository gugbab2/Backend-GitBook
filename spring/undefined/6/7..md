...
* 애플리케이션 서비스(application service) : 애플리케이션 계층에 존재하는 서비스&#x20;
  * `@Service` 애노테이션을 붙인다.&#x20;
* 도메인 서비스(domain service) : 특정 도메인 객체에 비지니스 로직을 포함하기 애매할 때 사용하는 서비스
* **인프라 서비스(infrastructure service) : 서비스 추상화의 대상!!**
