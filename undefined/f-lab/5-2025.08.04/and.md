...
그러나 **프로세스 자원 공유는 커널 프로세스와 컨텍스트 스위칭(Message Passing 방식)을 해야하기 때문에, 자원 부담이 크다는 단점이 있다.** 그래서 다중 작업이 필요한경우 스레드를 사용하는 것이 효율적이라, 현대 컴퓨터의 운영체제에선 다중 프로세싱을 지원하고 있지만 다중 스레딩을 기본으로 하고 있다.&#x20;
...
이 상황에서 수십수백개의 프로세스를 돌리기 위해서는 결국 동시성(**논리적인 효율**)이 필요한다.
...
프로세스와 마찬가지로, 스레드에도 상태가 이싿. 다음과 같은 4가지 상태를 가진다.&#x20;
