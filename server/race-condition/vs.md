...
* 특히나 프로세스 Context Swiching 을 하면 CPU 는 다음 프로세스의 정보를 불러오기 위해 메모리를 검색하고, CPU 캐시 메모리를 초기화하며, 프로세스 상태를 저장하고, 불러올 데이터를 준비해야 하기 때문에, 빈번한 Context Swiching 작업은 비용으로 인한 오버헤드가 발생할 수밖에 없다...
